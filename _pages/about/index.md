...
Israh is a responsive Jekyll theme with a clean single column layout and not much else. Most notably, Israh takes the focus off of header images so I don't have to find them for posts that don't need them.
...
This page is mostly a copy of [the one from "The Plain" Jekyll Theme](https://github.com/heiswayi/the-plain){:target="_blank"}.
...
Let's start with a informative paragraph. **This text is bolded.** But not this one! _How about italic text?_ Cool right? Ok, let's **_combine_** them together. Yeah, that's right! I have code to highlight, so `ThisIsMyCode()`. What a nice! Good people will hyperlink away, so [here we go](#) or [http://www.example.com](http://www.example.com){:target="_blank"}.
